...
export const PostLike = (props: any) => {
  const { user, postId } = props;
  const [likes, setLikes] = useState<ILike[] | null>(null);
  const addLikeHandler = () => {
    addLike(user, postId).then((res: any) => {
      if (user) {
        setLikes((prev) => (prev ? [...prev, res] : [res]));
      }
    });
  };
  const removeLikeHandler = () => {
    removeLike(user, postId).then((res: any) => {
      if (user) {
        setLikes((prev) => prev && prev.filter((like) => like.likeId !== res));
      }
    });
  };
  ...
  return (
    <>
      <button
        ...
        onClick={isUserLiked ? removeLikeHandler : addLikeHandler}
      >
        {isUserLiked ? (
          <>
            <svg
